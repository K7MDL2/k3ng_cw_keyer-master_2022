/*
 ...
 Copyright 2010 - 2021 Anthony Good, K3NG
...
English code training word lists from gen_cw_words.pl by Andy Stewart, KB1OIQ
 ...
 Command Line Interface ("CLI") (USB Port) (Note: turn on carriage return if using Arduino Serial Monitor program)
    ...
    \:     Extended CLLI commands
            ...
            wifi <ssid> <password>  - Set wifi ssid and password for wifi accesspoint
...
#include "keyer_debug.h"
 ...
 // #include <BasicTerm.h>
  ...
  #include <goertzel.h>
#endif
#if defined(FEATURE_WIFI) && defined(HARDWARE_ESP32_DEV) //SP5IOU 20220127
#include <WiFi.h>
...
#if !defined(ARDUINO_MAPLE_MINI) && !defined(ARDUINO_GENERIC_STM32F103C) && !defined(HARDWARE_ESP32_DEV)//sp5iou 20180329 20220127
...
void print_serial_help(PRIMARY_SERIAL_CLS * port_to_use,byte paged_help){
  ...
  port_to_use->println(F("\\+\t\t: Prosign the next two characters"));//Added "the next two characters" (WD9DMP)
    ...
    port_to_use->println(F("\t\twifi <ssid> <password> \t- Set wifi ssid and password for wifi accesspoint"));
...
#if defined(FEATURE_WIFI) //SP5IOU 20220128
  String userinputNoUpper = userinput;
  userinput.toUpperCase();
  if (userinput.startsWith("WIFI ")) { cli_wifi(port_to_use, userinputNoUpper.substring(5)); return; }
 else { userinput.toUpperCase();}
#endif
...
#if defined(FEATURE_WIFI) //SP5IOU 20220128
  if (userinput.startsWith("WIFI ")) { cli_wifi(port_to_use, userinput.substring(5)); return; }
#endif
...
#endif
//---------------------------------------------------------------------
#if defined(FEATURE_SERIAL) && defined(FEATURE_COMMAND_LINE_INTERFACE) && !defined(OPTION_EXCLUDE_EXTENDED_CLI_COMMANDS) && defined(FEATURE_WIFI)
void cli_wifi(PRIMARY_SERIAL_CLS* port_to_use, String command_arguments) {
#if defined(DEBUG_WIFI)
    debug_serial_port->print(F("\ncommand_arguments ")); //DEBUG
    debug_serial_port->print(command_arguments); //DEBUG
    debug_serial_port->print(F(" Sizeof command_arguments "));//DEBUG
    debug_serial_port->println(command_arguments.length()); //DEBUG
#endif
    int index;
    for (index = 0; (index < command_arguments.length()) && (command_arguments[index]!=' '); index++) {
        configuration.wifissid[index] = command_arguments[index];
        }
    configuration.wifissid[index] = 0; //Insert NULL as string terminantion
    String trimmer = configuration.wifissid;
     int index1 = index + 1;
#if defined(DEBUG_WIFI)
    debug_serial_port->print(F("\nconfiguration.wifissid "));
    debug_serial_port->print(configuration.wifissid); //DEBUG
    debug_serial_port->print(F(" index1 "));
    debug_serial_port->print(index1); //DEBUG
#endif
    for (index = index1; (index < command_arguments.length())/* && (command_arguments[index] != ' ')*/; index++) {
        configuration.wifipassword[index-index1] = command_arguments[index];
    }
    configuration.wifipassword[index-index1] = 0;//Insert NULL as string terminantion
...
//    port_to_use->print(F("Ethernet: "));
    port_to_use->print(F("IP Address: ")); //SP5IOU 20220129
...
boolean wifiConnect(char* ssid = WIFI_SSID, char* password = WIFI_PASSWORD, int attempts = 20) {
...
#endif  
        for (int i = 0; i < 4; i++) {
            configuration.ip[i] = WiFi.localIP()[i];
            configuration.gateway[i] = WiFi.gatewayIP()[i];
            configuration.subnet[i] = WiFi.subnetMask()[i];
    ...
    {
        for (int i = 0; i < 4; i++) {
            configuration.ip[i] = 0;
            configuration.gateway[i] = 0;
            configuration.subnet[i] = 0;
            configuration.dns_server[i] = WiFi.dnsIP()[i];
        }
        WiFi.disconnect();
#if defined DEBUG_WIFI
...
#else //SP5IOU 20220129
        configuration.ip[x] = WiFi.localIP()[x];
        configuration.gateway[x] = WiFi.gatewayIP()[x];
        configuration.subnet[x] = WiFi.subnetMask()[x];
#endif //SP5IOU 20220129
...
void initialize_ethernet(){
  ...
  #if defined(FEATURE_ETHERNET) && !defined(FEATURE_WIFI) //SP5IOU 220129
...
void web_print_page_about(EthernetClient client){
  ...
  #if !defined(HARDWARE_GENERIC_STM32F103C) && !defined(HARDWARE_ESP32_DEV)
    void* HP = malloc(4);
    if (HP){
      free (HP);
    }
